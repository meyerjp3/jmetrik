...
public class RankingProcess extends AbstractJmetrikProcess {

    private final String PROCESS_NAME = "Ranking Process";
    ...
    public void setCommand(Command command){
        this.command = (RankingCommand)command;
    }
    ...
    public void addMenuItem(final JFrame parent, JMenu menu, final TreeMap<String, JDialog> dialogs, final Workspace workspace, final JList tableList){
        ...
        JMenuItem menuItem = new JMenuItem("Ranking...");
        menuItem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(!workspace.databaseOpened()){
                    ...
                    RankingDialog rankingDialog = (RankingDialog)dialogs.get(PROCESS_NAME);
                    if(rankingDialog==null){
                        rankingDialog = new RankingDialog(parent, workspace.getDatabaseName(), tableName, workspace.getVariables());
                        dialogs.put(PROCESS_NAME, rankingDialog);
                        workspace.addVariableChangeListener(rankingDialog.getVariableChangedListener());
                    }
                    rankingDialog.setVisible(true);

                    if(rankingDialog.canRun()){
                        workspace.runProcess(rankingDialog.getCommand());
                    }
                }
            }
        });
    ...
    public void runProcess(Connection conn, JmetrikDatabaseFactory dbFactory, final JTabbedPane tabbedPane, ThreadPoolExecutor threadPool){
        ...
        JmetrikTab jTab = new JmetrikTab("rank"+ tabCount);
        ...
        RankingAnalysis rankingAnalysis = new RankingAnalysis(conn, dbFactory.getDatabaseAccessObject(), command, textFile);

        for(PropertyChangeListener pcl : propertyChangeListeners){
            textFile.addPropertyChangeListener(pcl);
            rankingAnalysis.addPropertyChangeListener(pcl);
        }

        for(VariableChangeListener vcl : variableChangeListeners){
            rankingAnalysis.addVariableChangeListener(vcl);
        }

        threadPool.execute(rankingAnalysis);
